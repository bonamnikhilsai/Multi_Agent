...
import AgentTimeline, { AgentTurn } from './AgentTimeline';
...
const DebateArena: React.FC = () => {
  ...
  const [debateHistory, setDebateHistory] = useState<Array<{ query: string; sessionId: string }>>([]);

  // Debate progress tracking
  const [currentRound, setCurrentRound] = useState(0);
  const [totalRounds] = useState(3);
  const [debatePhase, setDebatePhase] = useState<'idle' | 'debating' | 'synthesizing' | 'complete'>('idle');
  const [lastSubmittedQuery, setLastSubmittedQuery] = useState('');

  const textareaRef = useRef<HTMLTextAreaElement>(null);
  const accumulatedContent = useRef<{ [role: string]: string }>({});
  const socketRef = useRef<Socket | null>(null);
  // track role transitions to count rounds
  const turnRoleTracker = useRef<string[]>([]);
  ...
  const connectSocket = useCallback(() => {
    ...
    s.on('connected', (data: { message: string; user: { username: string } }) => {
      setStatusMessage(`Welcome back, ${data.user.username}`);
    });

    s.on('session_created', ({ sessionId: sid }: { sessionId: string }) => {
      setSessionId(sid);
    });

    s.on('debate_started', ({ query: q }: { query: string; sessionId: string }) => {
      setIsDebating(true);
      setTurns([]);
      setCurrentRound(1);
    ...
    s.on('agent_turn_start', ({ role }: { role: string }) => {
      setCurrentTurnRole(role);
      setTurns(prev => [...prev, { role: role as AgentTurn['role'], content: '', isStreaming: true }]);
    ...
    s.on('agent_chunk', ({ role, content }: { role: string; content: string }) => {
      accumulatedContent.current[role] = (accumulatedContent.current[role] || '') + content;

      setTurns(prev => {
        const updated = [...prev];
        const lastIdx = updated.map(t => t.role).lastIndexOf(role as AgentTurn['role']);
        if (lastIdx !== -1) {
          updated[lastIdx] = {
            ...updated[lastIdx],
    ...
    s.on('debate_complete', ({ sessionId: sid }: { sessionId: string }) => {
  ...
  const handleSubmit = useCallback((e?: React.FormEvent) => {
    ...
    if (!s || !isConnected || !query.trim() || isDebating) return;
    setErrorMessage('');
  ...
  return (
    <div className="arena-layout">
      ...
      <header className="arena-navbar glass-card" style={{ borderRadius: 0, borderLeft: 'none', borderRight: 'none', borderTop: 'none' }}>
        ...
        <div className="navbar-center">
          ...
          {!isConnected && !isConnecting && (
            <button className="btn-reconnect" onClick={connectSocket} id="reconnect-btn" title="Reconnect to gateway">
              ↻ Reconnect
            </button>
          )}
      ...
      {/* ── Debate Progress Bar ── */}
      {debatePhase !== 'idle' && (
        <div className="debate-progress-bar-wrapper">
          ...
          <div className="debate-progress-info">
            ...
            {debatePhase === 'synthesizing' && (
              <span className="active-role-label active-role--synthesizer">🧠 Synthesizer delivering final verdict…</span>
            )}
            {debatePhase === 'complete' && (
              <span className="active-role-label active-role--complete">✅ Debate complete — {totalRounds} rounds</span>
            )}
          </div>
        </div>
      )}
        ...
        <section className="arena-left-pane">
          <div className="pane-header">
            ...
            <div className="pane-header-right">
              {isDebating && currentTurnRole && (
                <div className={`active-agent-badge badge badge-${currentTurnRole}`}>
          ...
          {lastSubmittedQuery && (
            <div className="submitted-query-bar">
        ...
        <section className="arena-right-pane">
          ...
          <div className="query-panel glass-card">
            ...
            <form onSubmit={handleSubmit} id="debate-form">
              <textarea
                ref={textareaRef}
                id="debate-query-input"
                className="input-field query-textarea"
                placeholder={"Describe your architectural dilemma…\n\ne.g. Should I use GraphQL or REST for my mobile app?"}
                value={query}
              ...
              <div className="query-footer">
                ...
                <button
                  ...
                  disabled={!isConnected || !query.trim() || isDebating}
                >
                  {isDebating ? (
                    <><div className="spinner" /> Debating…</>
                  ) : (
                    <><span>⚔️</span> Launch Debate</>
                  )}
                </button>
              </div>
            </form>
            ...
            {errorMessage && (
              <div className="error-alert animate-fadeIn" role="alert" style={{ marginTop: 12 }}>
                <span>⚠</span>
                <span style={{ flex: 1 }}>{errorMessage}</span>
                {!isConnected && (
                  <button className="btn-retry" onClick={connectSocket} id="retry-connect-btn">Retry</button>
                )}
              </div>
            )}
          </div>
            ...
            <div className="round-dots">
              {Array.from({ length: totalRounds }).map((_, i) => (
                <div
                  key={i}
                  className={`round-dot ${
                    debatePhase === 'complete' || (debatePhase === 'debating' && i < currentRound)
                      ? 'round-dot--done'
          ...
          {debateHistory.length > 0 && (
            <div className="history-panel glass-card">
              ...
              <div className="history-list">
                {debateHistory.slice(0, 5).map((h, i) => (
          ...
          <div className="legend-panel glass-card">
            ...
            <div className="legend-list">
              {AGENT_ROSTER.map(a => (
                <div key={a.role} className={`legend-item legend-item--${a.role}`}>
