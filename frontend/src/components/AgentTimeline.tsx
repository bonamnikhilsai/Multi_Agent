...
export interface AgentChunk {
  role: 'researcher' | 'critic' | 'synthesizer' | 'system' | 'error';
  content: string;
  sessionId?: string;
}

export interface AgentTurn {
  role: AgentChunk['role'];
  content: string;
  isStreaming?: boolean;
}
...
interface CriticSection {
  topic: string;
  targetedClaim: string;
  counterArguments: string[];
  risksAndFailures: string[];
  alternative: string;
  painPointsExposed: string[];
  riskSeverity: 'LOW' | 'MEDIUM' | 'HIGH' | 'CRITICAL' | '';
  raw: string;
}

interface SynthesizerSection {
  topic: string;
  winner: string;
  finalVerdict: string;
  winningReasons: string[];
  strongestDebatePoints: string[];
  painPointsAndMitigation: string[];
...
const extractInlineField = (text: string, fieldName: string): string => {
...
const parseResearcher = (content: string): ResearcherSection => {
  const headers = [
    'Topic',
    'Current Position',
    'Key Advantages',
  ...
  const getSection = (h: string) => extractSection(content, h, headers.filter(x => x !== h));
...
const parseCritic = (content: string): CriticSection => {
  const headers = [
    'Topic',
    'Targeted Claim',
    'Counter Arguments',
  ...
  const severity = (severityMatch?.[1]?.toUpperCase() ?? '') as CriticSection['riskSeverity'];
...
const parseSynthesizer = (content: string): SynthesizerSection => {
  const headers = [
    'Topic',
    'Winner',
    'Final Verdict',
...
const ConfidenceMeter: React.FC<{ score: string; colorClass: string }> = ({ score, colorClass }) => {
  ...
  return (
    <div className="confidence-meter">
      ...
      <div className="confidence-bar-track">
        <div
          className={`confidence-bar-fill confidence-bar-fill--${colorClass}`}
...
const RiskBadge: React.FC<{ severity: CriticSection['riskSeverity'] }> = ({ severity }) => {
  ...
  return (
    <div className={`risk-badge ${colors[severity] ?? ''}`}>
...
const SectionHeader: React.FC<{ icon: string; title: string; colorClass: string }> = ({ icon, title, colorClass }) => (
  <div className={`section-header section-header--${colorClass}`}>
    <span className="section-header-icon">{icon}</span>
    <span className="section-header-title">{title}</span>
  </div>
);

const BulletList: React.FC<{ items: string[]; colorClass: string; variant?: 'risk' | 'pain' | 'default' }> = ({
  items,
  colorClass,
  variant = 'default',
}) => {
  ...
  return (
    <ul className={`structured-bullets structured-bullets--${variant}`}>
      {items.map((item, i) => (
        <li key={i} className={`structured-bullet structured-bullet--${colorClass}`}>
...
const PainPointChips: React.FC<{ items: string[]; colorClass: string }> = ({ items, colorClass }) => {
  if (!items.length) return null;
  return (
    <div className="pain-chips">
      {items.map((item, i) => (
        <span key={i} className={`pain-chip pain-chip--${colorClass}`}>
...
const ResearcherCard: React.FC<{ parsed: ResearcherSection; isStreaming?: boolean }> = ({ parsed, isStreaming }) => {
  const isStructured = parsed.keyAdvantages.length > 0 || parsed.currentPosition.length > 0;

  if (!isStructured || isStreaming) {
    return (
      <div className={`agent-text ${isStreaming ? 'agent-text--streaming' : ''}`}>
  ...
  return (
    <div className="structured-output">
      {parsed.topic && (
        <div className="topic-label topic-label--researcher">
      ...
      {parsed.defense && !parsed.defense.toLowerCase().includes('n/a') && (
        <div className="structured-section">
      ...
      {parsed.painPointsAddressed.length > 0 && (
        <div className="structured-section">
...
const CriticCard: React.FC<{ parsed: CriticSection; isStreaming?: boolean }> = ({ parsed, isStreaming }) => {
  ...
  return (
    <div className="structured-output">
      {parsed.topic && (
        <div className="topic-label topic-label--critic">
          <span>📋</span> Topic: {parsed.topic}
        </div>
      )}

      {parsed.targetedClaim && (
        <div className="structured-section">
      ...
      {parsed.counterArguments.length > 0 && (
        <div className="structured-section">
      ...
      {parsed.risksAndFailures.length > 0 && (
        <div className="structured-section">
      ...
      {parsed.painPointsExposed.length > 0 && (
        <div className="structured-section">
...
const SynthesizerCard: React.FC<{ parsed: SynthesizerSection; isStreaming?: boolean }> = ({ parsed, isStreaming }) => {
  ...
  return (
    <div className="structured-output">
      {parsed.topic && (
        <div className="topic-label topic-label--synthesizer">
          <span>📋</span> Topic: {parsed.topic}
        </div>
      )}
      ...
      {parsed.finalVerdict && (
        <div className="structured-section verdict-section">
      ...
      {parsed.winningReasons.length > 0 && (
        <div className="structured-section">
      ...
      {parsed.strongestDebatePoints.length > 0 && (
        <div className="structured-section">
      ...
      {parsed.painPointsAndMitigation.length > 0 && (
        <div className="structured-section">
      ...
      {parsed.productionArchitecture.length > 0 && (
        <div className="structured-section">
      ...
      {parsed.raw.includes('```') && !isStreaming && (
        <CodeSandbox content={parsed.raw} label="JavaScript Boilerplate" />
      )}

      {parsed.implementationRoadmap.length > 0 && (
        <div className="structured-section">
...
const AgentTurnCard: React.FC<{ turn: AgentTurn; index: number }> = ({ turn, index }) => {
  ...
  return (
    <div
      className={`agent-turn agent-turn--${config.colorClass} animate-slideInLeft`}
...
const AgentTimeline: React.FC<AgentTimelineProps> = ({ turns, isDebating }) => {
    ...
    return (
      <div className="timeline-empty">
        ...
        <div className="empty-agents">
          {(['researcher', 'critic', 'synthesizer'] as const).map(role => {
            const c = AGENT_CONFIG[role];
            return (
              <div key={role} className={`empty-agent-card empty-agent-card--${role}`}>
  ...
  const renderTurns = () => {
    ...
    for (let i = 0; i < turns.length; i++) {
      ...
      if (t.role === 'researcher' || t.role === 'critic') {
        ...
        if (isLastInRound) {
          elements.push(
            <div key={`round-${roundNum}`} className="debate-round-box animate-fadeIn">
              <div className="round-header">
                <div className="round-badge-pill">Round {roundNum}</div>
                <div className="round-line" />
              </div>
              {roundBuffer.map((rt, idx) => (
                <AgentTurnCard key={`${rt.role}-${i}-${idx}`} turn={rt} index={idx} />
              ))}
            </div>
          );
          roundBuffer = [];
        ...
        if (t.role === 'synthesizer') {
          elements.push(
            <div key={`synth-${i}`} className="synthesizer-verdict-wrapper animate-fadeIn">
