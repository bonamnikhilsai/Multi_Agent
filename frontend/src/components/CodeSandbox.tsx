...
const extractAllCodeBlocks = (text: string): CodeBlock[] => {
...
const formatCodeWithLineNumbers = (code: string): { num: number; line: string }[] => {
  return code.split('\n').map((line, i) => ({ num: i + 1, line }));
};

const CodeSandbox: React.FC<CodeSandboxProps> = ({ content, label }) => {
    ...
    <div className="code-sandbox">
      ...
      <div className="code-sandbox-header">
        ...
        {codeBlocks.length > 1 ? (
          ...
          <div className="code-sandbox-label">
        ...
        <div className="code-sandbox-actions">
          ...
          <button
            id={`copy-code-btn-${activeBlock.index}`}
            className={`copy-btn ${copied === activeBlock.index ? 'copy-btn--copied' : ''}`}
            onClick={() => handleCopy(activeBlock.code, activeBlock.index)}
            aria-label="Copy code to clipboard"
            title="Copy code"
          >
            {copied === activeBlock.index ? (
              <><span>✓</span> Copied!</>
            ) : (
              <><span>⧉</span> Copy</>
            )}
      ...
      <div className="code-sandbox-body">
        <table className="code-table">
          <tbody>
            {lines.map(({ num, line }) => (
              <tr key={num} className="code-line">
                {showLineNumbers && (
                  <td className="code-line-num">{num}</td>
                )}
